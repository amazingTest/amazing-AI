...
def get_search_words_and_retention():
    import pandas as pd
    csv_data = pd.read_csv('./data/sentiment_chinese.txt')
    text = list(csv_data['text'])
    labels = list(map(lambda x: 0 if x == 'negative' else 1, csv_data['labels']))
    return vectorize(text), labels
...
if __name__ == '__main__':
    q, p = get_search_words_and_retention()
    print(len(p))
